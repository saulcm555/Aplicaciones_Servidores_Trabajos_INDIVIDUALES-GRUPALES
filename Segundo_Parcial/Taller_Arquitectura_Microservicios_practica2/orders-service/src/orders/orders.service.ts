...
export class OrdersService {
  ...
  constructor(
    ...
    private readonly rabbitPublisher: RawRabbitPublisher,
  ) {}
  ...
  async handleStockReserved(data: {
    ...
    try {
      ...
      if (data.approved) {
        this.logger.log(`📤 Publishing event: order.confirmed for order ${order.id}`);
        await this.rabbitPublisher.publish('order.confirmed', eventPayload);
      } else {
        this.logger.log(`📤 Publishing event: order.cancelled for order ${order.id}`);
        await this.rabbitPublisher.publish('order.cancelled', {
          ...eventPayload,
