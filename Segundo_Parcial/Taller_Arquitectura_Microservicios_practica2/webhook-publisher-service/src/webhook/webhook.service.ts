...
export class WebhookService {
   ...
   * NUEVA ESTRATEGIA DE IDEMPOTENCIA:
   * - Clave compuesta: eventName:idempotencyKey:subscriberId
   * - Permite que el mismo evento vaya a múltiples subscribers
   * - Evita duplicados por subscriber individual
   */
  async processEvent(eventName: string, eventData: any): Promise<void> {
    ...
    // Encolar webhooks para cada suscriptor (con deduplicación individual)
    for (const subscriber of subscribers) {
      // Verificar idempotencia POR SUBSCRIBER
      ...
      if (alreadyProcessed) {
        this.logger.warn(
          `⚠️ Event already processed for subscriber: ${subscriber.name} | Event: ${eventName} | Key: ${idempotencyKey}`,
        );
        continue; // Saltar este subscriber
      }

      // Marcar como procesado ANTES de encolar (evita duplicados en reintento)
      ...
      await this.webhookQueue.add(`webhook-${subscriber.name}`, job, {
        jobId: `${idempotencyKey}-${subscriber.id}`, // ID único para idempotencia en cola
      });
