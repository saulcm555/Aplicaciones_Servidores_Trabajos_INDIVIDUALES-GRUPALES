...
// CON DEDUPLICACIÓN FINAL (7 días)
// ========================================
...
serve(async (req) => {
  ...
  try {
    ...
    if (!payload.event || !payload.data || !payload.idempotency_key) {
      return new Response(
    ...
    // ========================================
    // DEDUPLICACIÓN FINAL (7 días) - RPC atómica
    // ========================================
    
    const { data: wasInserted, error: rpcError } = await supabase
      .rpc('mark_webhook_as_processed', {
        p_idempotency_key: payload.idempotency_key,
        p_event_name: payload.event,
        p_metadata: {
          data: payload.data || {},
          timestamp: payload.timestamp,
          source: payload.metadata?.source || 'webhook-publisher',
          processor: 'telegram-notifier'
        }
      });

    if (rpcError) {
      console.error('❌ Error en RPC mark_webhook_as_processed:', rpcError);
      return new Response(
        JSON.stringify({ 
    ...
    // wasInserted = FALSE → Duplicado, ya fue procesado
    if (!wasInserted) {
      console.warn(`⚠️ Webhook duplicado ignorado: ${payload.event} | Key: ${payload.idempotency_key}`);
      return new Response(
        JSON.stringify({ 
          success: true,
          status: 'duplicate',
          ...
          event: payload.event,
          note: 'Notification already sent'
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`✅ Primera vez procesando: ${payload.event} | Key: ${payload.idempotency_key}`);
